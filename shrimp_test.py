...
from ShrimpEstimator.tools.tools import ShrimpModelV1, ShrimpModelV2, ShrimpDataset
...
def predict_single(model, subject: str, device: str, validate_set : bool = False):
    if not validate_set:
        X_path = fr'Shrimp\data\shrimp_ds\jpeg\shrimp_{subject}.jpg'
    else:
        X_path = fr'Shrimp\data\validate_set\jpeg\shrimp_{subject}.jpg'

    y_path = fr'Shrimp\data\shrimp_ds\hm\shrimp_{subject}.npy'
    
    test_ds = ShrimpDataset(x=[X_path], y=[np.load(y_path)])
    X, _ = test_ds[0]
...
if __name__ == '__main__':
    ...
    DEV = 'cpu'
    BASE_CH = 32
    MODEL_PATH = fr"Shrimp\models\best_shrimp_model_{BASE_CH}.pth"
